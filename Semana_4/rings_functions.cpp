...
void write_time(Mat &frame, int64 start, double &tda, int nframes) {
  ...
  fps =  tda/nframes;
  sstream << fps;
  putText(frame, sstream.str(), Point(20, 40), 1, 1.5, Scalar(255,0,0), 2,0);
}

void numbering(Mat &frame, vector<Point2f> arrange) {
  stringstream ss;
  for (size_t i = 0; i < arrange.size(); i++){
    ss << i;
    putText(frame, ss.str(), arrange[i], 1.7, 2.0, Scalar(0,0,0), 0.1, 0);
    ss.str("");
  }
}
...
void fill_arrange(vector<Point2f> input, vector<Point2f> &output, bool next_row,
                             vector<int> &corners, Point2f axis_x,
                             float dis_x, Point2f axis_y, float dis_y){
