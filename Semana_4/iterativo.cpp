...
void frontImageChess( Mat &lambda, Mat rview, Mat &output, vector<Point2f> &corners, Size patternsize){
  Point2f inputQuad[4];
  Point2f outputQuad[4];
  ...
  outputQuad[1] = Point2f( 560 - 1,80);
  outputQuad[2] = Point2f( 560 - 1,400 - 1);
  outputQuad[3] = Point2f( 80,400 - 1);
  lambda = getPerspectiveTransform( inputQuad, outputQuad );
  warpPerspective(rview,output,lambda,output.size() );
  lambda = getPerspectiveTransform( outputQuad, inputQuad );
}

void frontImageAsime( Mat &lambda, Mat rview, Mat &output, vector<Point2f> &corners, Size patternsize){
  Point2f inputQuad[4];
  Point2f outputQuad[4];

  inputQuad[0] = corners[patternsize.width - 1];
  inputQuad[1] = corners[(patternsize.height*patternsize.width) - 1];
  inputQuad[2] = corners[((patternsize.height - 1)*patternsize.width)];
  inputQuad[3] = corners[0];

  outputQuad[0] = Point2f( 100, 100 );
  outputQuad[1] = Point2f( 550 - 1, 100);
  outputQuad[2] = Point2f( 550 - 1,330 - 1);
  outputQuad[3] = Point2f( 100,330 - 1);
  lambda = getPerspectiveTransform( inputQuad, outputQuad );
  warpPerspective(rview,output,lambda,output.size() );
  lambda = getPerspectiveTransform( outputQuad, inputQuad );
}
